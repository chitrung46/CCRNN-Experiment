...
def evaluate(predictions, targets, normal):
    ...
    for horizon in range(12):
        ...
        scores['MAPE'][f'horizon-{horizon}'] = normal.mape_transform(mean_absolute_percentage_error(y_true, y_pred))
    ...
    scores['mape'] = normal.mape_transform(mean_absolute_percentage_error(y_true, y_pred))
    return scores
...
def mask_evaluate(predictions: np.ndarray, targets: np.ndarray, normal):
    """
    ...
    for horizon in range(12):
        y_true = np.reshape(targets[:, horizon], (n_samples, -1))
        y_pred = np.reshape(predictions[:, horizon], (n_samples, -1))
        scores['masked MAE'][f'horizon-{horizon}'] = normal.mae_transform(masked_mae_np(y_pred, y_true, null_val=0.0))
        scores['masked RMSE'][f'horizon-{horizon}'] = normal.rmse_transform(masked_rmse_np(y_pred, y_true, null_val=0.0))
        scores['masked MAPE'][f'horizon-{horizon}'] = normal.mape_transform(masked_mape_np(y_pred, y_true, null_val=0.0))
    y_true = np.reshape(targets, (n_samples, -1))
    y_pred = np.reshape(predictions, (n_samples, -1))
    scores['loss'] = masked_rmse_np(y_pred, y_true, null_val=0.0)
    scores['mae'] = masked_mae_np(y_pred, y_true, null_val=0.0)
    scores['mape'] = masked_mape_np(y_pred, y_true, null_val=0.0) * 100.0
