...
from utils.evaluate import mask_evaluate
    ...
    try:
        for epoch in range(begin_epoch, begin_epoch + epochs):
            ...
            for phase in phases:
                ...
                # 性能
                running_metrics[phase] = mask_evaluate(np.concatenate(predictions), np.concatenate(running_targets), normal[0])
                # print(running_metrics[phase]['loss'])
                running_metrics[phase].pop('loss')
                running_metrics[phase].pop('mae')
                running_metrics[phase].pop('mape')
...
def test_model(folder: str,
    ...
    scores = mask_evaluate(running_targets, predictions, normal[0])
